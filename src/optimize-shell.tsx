...
import { ActionPanel, Action, List, showToast, Toast, getPreferenceValues, Icon, confirmAlert, Alert } from "@raycast/api";
...
export default function OptimizeShell() {
    ...
    async function analyzeShell() {
        try {
            const configPath = preferences.shellType === "zsh" ? "~/.zshrc" : "~/.bashrc";
            ...
            const metrics = {
                ...
                functionCount: (configContent.match(/function\s+\w+|^\w+\(\)/gm) || []).length,
                ...
                historySize: fs.existsSync(`${homedir()}/.${preferences.shellType}_history`)
                    ? fs.statSync(`${homedir()}/.${preferences.shellType}_history`).size
                    : 0,
                completionCount: (configContent.match(/compdef|complete/g) || []).length
            };
...
[ -s "$NVM_DIR/lazy.sh" ] && . "$NVM_DIR/lazy.sh"`
    ...
    async function optimizeConfiguration() {
        const confirmed = await confirmAlert({
            ...
            message: "This will modify your shell configuration. A backup will be created. Continue?",
            primaryAction: {
                title: "Optimize",
...
setopt SHARE_HISTORY`;
            ...
            showToast({ style: Toast.Style.Success, title: "History settings optimized" });
        } catch (error) {
            showToast({ style: Toast.Style.Failure, title: "History optimization failed" });
        }
    }
...
zstyle ':completion:*' cache-path ~/.zsh/cache`;
            ...
            showToast({ style: Toast.Style.Failure, title: "Completion optimization failed" });
        }
    }
                ...
                {metrics && (
                    <>
                        <List.Item
                            ...
                            icon={metrics.startupTime > 500 ? Icon.ExclamationMark : Icon.Clock}
                            accessories={[{ text: metrics.startupTime > 500 ? "Slow" : "Good" }]}
                        />
                        <List.Item
                            title="Plugin Count"
                ...
                <List.Section title="Installed Plugins">
                    {plugins.map((plugin, index) => (
                        <List.Item
                            ...
                            title={plugin.name}
                            icon={
                                plugin.impact === "high" ? Icon.ExclamationMark :
                                    plugin.impact === "medium" ? Icon.QuestionMark :
                                        Icon.Checkmark
                            }
